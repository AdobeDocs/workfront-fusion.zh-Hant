...
- **是否為新的聯結器啟動** （全新的聯結器/應用程式即將推出，而不只是新增至現有聯結器的新模組）。 如果是，這會觸發&#x200B;**步驟7** — 請勿因為發行說明本身已完成，而略過詢問重新導向。
...
## 步驟7：新聯結器啟動 — 詢問重新導向（請勿略過）

**每當步驟1識別出新的聯結器啟動時，就會套用此步驟。** 在步驟5之後很容易將發行說明視為「完成」，而忘記這一點 — 將新的聯結器功能視為不完整，直到以某種方式解決此步驟為止。

詢問使用者： *「您要為新的聯結器文章設定重新導向嗎？」*

- 如果&#x200B;**no**，請注意並繼續進行 — 沒有其他可做的事。
- 如果&#x200B;**是**，請收集：
  - **來源路徑** （必須以`/en`開頭，不含空格）
...
- [ ] **如果任何功能是新的聯結器啟動：詢問關於重新導向（步驟7），然後設定或明確拒絕**
