---
title: Workfront Proof 模組
description: 在 Adobe Workfront Fusion 情境中，您可以將使用  [!DNL Workfront Proof] 的工作流程自動化，以及將其連接至多個第三方應用程式和服務。
author: Becky
...
在 Adobe Workfront Fusion 情境中，您可以將使用 [!DNL Workfront Proof] 的工作流程自動化，以及將其連接至多個第三方應用程式和服務。
...
## 將 [!DNL Workfront Proof] 連接至 Workfront Fusion
...
模組會傳回與校訂相關聯的所有標準欄位，以及連線存取的任何自訂欄位和值。 它也會為PDF摘要建立新的事件訂閱，並輸出來自承載中傳送的`pdf_url`屬性的內容。 您可以在情境內之後的模組中對應此資訊。
...
模組會傳回在指定期間找到的所有記錄清單，以及它們的型別。 它也會傳回您指定之欄位的值。 如果模組發現校訂上有決策，則會在個別欄位中同時包含先前和目前值。 您可以在情境內之後的模組中對應此資訊。
...
模組會傳回新校訂或校訂版本的ID。 您可以在情境內之後的模組中對應此資訊。
...
模組會傳回您為校樣選擇的欄位值及其型別。 您可以在情境內之後的模組中對應此資訊。
...
模組會傳回PDF摘要資訊。 您可以在情境內之後的模組中對應此資訊。
