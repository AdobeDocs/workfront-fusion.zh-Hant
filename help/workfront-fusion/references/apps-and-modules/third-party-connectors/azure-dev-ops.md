---
title: Azure DevOps 模組
description: 在 Adobe Workfront Fusion 情境中，您可以將使用  [!DNL Azure DevOps] 的工作流程自動化，以及將其連接至多個第三方應用程式和服務。
author: Becky
...
在 Adobe Workfront Fusion 情境中，您可以將使用 [!DNL Azure DevOps] 的工作流程自動化，以及將其連接至多個第三方應用程式和服務。
...
當您設定 [!DNL Azure DevOps] 模組時，Workfront Fusion 會顯示下列欄位。 除了這些欄位以外，也可能因為其他因素，例如您在應用程式或服務中的存取層級，而顯示其他 [!DNL Azure DevOps] 欄位。 在模組中，粗體標題表示那是必要欄位。
...
此即時觸發模組會在[!UICONTROL Azure DevOps]中新增、更新或刪除記錄時執行案例。
...
您可以利用此動作模組，對 [!DNL Azure DevOps] API 進行已驗證的自訂呼叫。 如此一來，您就可以建立其他 [!DNL Azure DevOps] 模組無法完成的資料流程自動化。
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr data-mc-conditions=""> 
   ...
   <td>選取或對應您要連線至此API呼叫的[!DNL Azure DevOps] API版本。 如果未選取版本，Workfront Fusion會連線至[!DNL Azure DevOps] API 5.1版。</td> 
  </tr> 
  <tr> 
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td>選取要包含在此模組的輸出組合包中的資訊。 可用欄位取決於工作專案型別。</td> 
  </tr> 
  <tr> 
...
模組會傳回主要工作專案的ID及任何關聯欄位，以及連線存取的任何自訂欄位和值。 您可以在情境內之後的模組中對應此資訊。
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td>輸入或對映Workfront Fusion在一個執行週期內傳回的最大工作專案數。</td> 
  </tr> 
 </tbody> 
</table>
