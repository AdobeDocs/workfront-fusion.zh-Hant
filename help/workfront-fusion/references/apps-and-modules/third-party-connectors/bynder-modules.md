---
title: 旁觀器模組
description: 在Adobe Workfront Fusion案例中，您可以自動化使用 [!DNL Bynder]的工作流程，並將其連線至多個協力廠商應用程式和服務。
author: Becky
...
在Adobe Workfront Fusion案例中，您可以自動化使用[!DNL Bynder]的工作流程，並將其連線至多個協力廠商應用程式和服務。
...
* [在[!UICONTROL 中產生]使用者端識別碼[!UICONTROL 和]使用者端密碼 [!DNL Bynder]  （選擇性）](#generate-a-client-id-and-client-secret-in-bynder-optional)
...
### 在[!UICONTROL 中產生]使用者端識別碼[!UICONTROL 和]使用者端密碼[!DNL Bynder] （選擇性）

如果您要使用使用者端ID和使用者端密碼建立連線，可以從您的[!DNL Bynder]帳戶產生連線。 當您在[!DNL Bynder]中建立應用程式時，會產生使用者端識別碼和使用者端密碼。

如需在[!DNL Bynder]中建立應用程式的指示，請參閱[檔案中的](https://developer-docs.bynder.com/api/authentication-oauth2-oauth-apps/)Oauth 2.0應用程式[!DNL Bynder]。
...
當您設定[!DNL Bynder]模組時，Workfront Fusion會顯示下列欄位。 除了這些欄位以外，可能還會顯示其他[!DNL Bynder]欄位，視您在應用程式或服務中的存取層級等因素而定。 模組中的粗體標題表示必填欄位。
