...
description: 此Adobe Workfront Fusion模組可讓您設定具有HTTP使用者端憑證授權的HTTP要求，並將其提交至伺服器。 接收的HTTP回應隨後會包含在輸出套件組合中。
author: Becky
...
此Adobe Workfront Fusion模組可讓您設定具有HTTP使用者端憑證授權的HTTP要求，並將其提交至伺服器。 接收的HTTP回應隨後會包含在輸出套件組合中。
...
當您設定[!UICONTROL HTTP] > [!UICONTROL 發出使用者端憑證授權要求]模組時，Adobe Workfront Fusion會顯示下列欄位。 在模組中，粗體標題表示那是必要欄位。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>選取包含您使用者端憑證驗證認證的金鑰，或按一下<strong>[!UICONTROL 新增]</strong>將您的認證新增至新的金鑰。 </p> <p>注意：您可以新增更多認證，以輕鬆地在每個連線之間切換。</p>           <p>若要擷取您的私密金鑰或憑證：</p>
          <ol>
            <li value="1">
    ...
    <ul> 
     ...
     <li> <p><strong>[!UICONTROL Multipart/form-data]</strong> </p> <p>[!UICONTROL Multipart/form-data]是用於傳送檔案和資料的HTTP多部分要求。 它通常用於將檔案上傳到伺服器。</p> <p>新增要在請求中傳送的欄位。 每個欄位都必須包含索引鍵/值組。</p> 
      <ul> 
       ...
       <li> <p><strong>[!UICONTROL 檔案]</strong> </p> <p>輸入金鑰，並指定您要在要求內文中傳送的來源檔案。 從先前的模組中選取來源檔案，或對應檔案的名稱和資料。</p> </li> 
      </ul> </li> 
    </ul> </td> 
  </tr> 
  <tr> 
